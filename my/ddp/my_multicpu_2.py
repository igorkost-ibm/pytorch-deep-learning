...
def worker(rank: int, world_size: int, save_every: int, total_epochs: int, batch_size: int):
    ...
    loss_fn = nn.MSELoss()
    optimizer = torch.optim.SGD(model.parameters(), lr=(0.1 * world_size))
    Trainer(model, train_data, train_ds, test_ds, loss_fn, optimizer, rank, save_every).train(total_epochs)
...
if __name__ == "__main__":
    ...
    mp.spawn(worker, args=(args.world_size, args.save_every, args.total_epochs, args.batch_size), nprocs=args.world_size, join=True)
    end_time = time.time()
